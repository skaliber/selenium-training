...
Selenium.prototype.doSetContext = function(context) {
    /**
   * Writes a message to the status bar and adds a note to the driver-side
   * log.
   *
   * @param context
   *            the message to be sent to the driver
   */
...
Selenium.prototype.doAttachFile = function(fieldLocator,fileLocator) {
   ...
   *  grid configurations where the RC server driving the driver is not the same
...
Selenium.prototype.doCaptureEntirePageScreenshotToString = function(kwargs) {
    /**
    * Downloads a screenshot of the driver current window canvas to a
...
Selenium.prototype.doShutDownSeleniumServer = function(keycode) {
    /**
    * Kills the running Selenium Server and all driver sessions.  After you run this command, you will no longer be able to send
    * commands to the server; you can't remotely start the server once it has been stopped.  Normally
    * you should prefer to run the "stop" command, which terminates the current driver session, rather than
